...
function registerModalHandlers() {
  ...
  const btnSave = document.getElementById("btn-save-entrada");
  const modalEl = document.getElementById("scan-summary-modal");
  let bsModal = null;
  if (modalEl) bsModal = new bootstrap.Modal(modalEl, { backdrop: true });
  ...
  if (btnSave) {
    btnSave.addEventListener("click", async () => {
      ...
      try {
        ...
        await updateProductQuantitiesAdd(productsToUpdate);
